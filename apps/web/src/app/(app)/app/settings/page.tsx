...
import { getWorkspaceSnapshot, listWorkspaceApiKeys } from '@/lib/vowgrid/repository';

export default async function SettingsPage() {
  const [snapshot, apiKeys] = await Promise.all([getWorkspaceSnapshot(), listWorkspaceApiKeys()]);
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <Card>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3 text-sm leading-6 text-[var(--color-text-secondary)]">
              <p>
                Dashboard pages use a session token stored in an HttpOnly cookie on the web app.
              </p>
              <p>
              ...
              <p>
                Workspace owners and admins can now create, rotate, and revoke API keys from this
                surface.
              </p>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-3 text-sm leading-6 text-[var(--color-text-secondary)]">
              <p>
                Billing is now modeled internally in VowGrid, with Mercado Pago isolated as the
                payment provider layer.
              </p>
              <p>
                The full billing surface lives at <span className="mono">/app/billing</span>,
                including trial state, usage meters, and provider setup requirements.
              </p>
              <p>
                Checkout remains environment-driven: missing Mercado Pago env vars are called out
                explicitly instead of being hidden behind fake success states.
              </p>
            </div>
          </CardContent>
        </Card>

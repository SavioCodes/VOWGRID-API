...
This document is the human-readable API map for VowGrid. The canonical machine-readable reference remains the live Swagger UI:
...
Some routes do not require authentication, such as:
...
| `GET`  | `/v1/intents/:intentId`                     | session or API key | intent detail                             |
...
| `GET`   | `/v1/workspace/api-keys`                  | session | list API keys metadata    |
...
1. `docs/API_REFERENCE.md` for a grouped map
2. `docs/AGENT_INTEGRATION_GUIDE.md` for real examples
3. `packages/contracts` for exact request/response shapes

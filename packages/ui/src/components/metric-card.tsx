...
export function MetricCard({ label, value, trend, tone = 'accent', children }: MetricCardProps) {
  return (
    <Card className={cx('relative overflow-hidden')}>
      <div
        className={cx('pointer-events-none absolute inset-0 bg-linear-to-br', accentMap[tone])}
      />
      <CardContent className="relative space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-3xl font-semibold tracking-[-0.04em] text-[var(--color-text-primary)]">
            {value}
          </p>
          {children ? (
